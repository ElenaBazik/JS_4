...
function GiveTagPnumber() {
    ...
    for (i = 0; i < arrayOfTag.length; i++) {
        arrayOfTag[i].innerHTML = (i+1) + " " + arrayOfTag[i].innerHTML;
    }
}
// Task4
var count = 0,
...
function counter() {
    count++;
    result.innerHTML = count + " : раз нажали на кнопку.";
}
// Task 5
document.querySelector('#task5').addEventListener('click', pow);
...
var someArray = [1, 2, 3, 4, 5],
    ...
    arrLengshow = document.querySelector("#arrLengshow");
...
function returnSomeNumber() {
    ...
    if (task10 <= 7) {
        return ('число равно или меньше 7');
    } else if (task10 >= 10) {
